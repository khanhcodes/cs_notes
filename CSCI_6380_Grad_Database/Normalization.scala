 ...
 *           Includes partial implementations of the BCNF Decomposition Algorithm
...
class Normalization (r: Attrs, fd: VEC [FD]):
    ...
    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Find the first Functional Dependency (FD) that violates the BCNF rule that
     *  the LHS of all applicable FDs are superkeys.  Return null if none found.
    ...
    def find_not_bcnf (ri: Attrs, fd_ : VEC [FD] = fd): FD = boundary:
        for f <- fd_ if f.x ⊆ ri do                                // LHS must be in Ri
            if ! superkey (f.x, ri, fd_) then                      // not a superkey w.r.t. Ri (violates BCNF)
                val yy = f.y & ri                                  // yy is relevant RHS of f.y in Ri
                if yy.size > 0 then break (FD(f.x, yy))            // return FD: f.x -> yy
        null                                                       // none found
    end find_not_bcnf
     ...
     *  Caveat: Only considers FDs in F, ignoring F+ (=> INCOMPLETE IMPLEMENTATION)
    ...
    def bcnf_decomp (tree: BinTree [Attrs] = bcnf_root, ri: Attrs = r, fd_ : VEC [FD] = fd): Unit =
        val f = find_not_bcnf (ri, fd_)                            // FD that violates BCNF
